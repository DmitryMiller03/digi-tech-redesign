...
export default async function NewCategoryPage() {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-semibold text-slate-900">Новая категория</h1>

      <form action={createCategory} className="mt-6 space-y-4 rounded-2xl bg-white p-6 shadow-sm">
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700">
            Slug <span className="text-slate-400">(необязательно — сгенерируется из названия)</span>
          </label>
          <input
            name="slug"
            className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-600 focus:outline-none focus:ring-1 focus:ring-blue-600"
